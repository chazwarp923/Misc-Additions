 ...
 * @author Azanor
 * 
 *         The tile entity used by nodes in the vis energy network. A node is
 *         either a source (like an aura node), a transport relay or vis
 *         receiver (like the infernal furnace).
 *
 */
public abstract class TileVisNode extends TileThaumcraft {
	...
	/**
	 * @return the number of blocks away this node will check for parent nodes
	 *         to connect to.
	 */
	...
	/**
	 * This method should never be called directly. Use VisNetHandler.drainVis()
	 * instead
	 * 
	 * @param aspect
	...
	public void removeThisNode() {
		...
		if (this.isSource()) {
			...
			VisNetHandler.sources
					.put(worldObj.provider.dimensionId, sourcelist);
		}
	...
	public WeakReference<TileVisNode> getRootSource() {
		return VisNetHandler.isNodeValid(getParent()) ? getParent().get()
				.getRootSource() : this.isSource() ? new WeakReference(this)
				: null;
	}
		...
		if (!worldObj.isRemote && ((nodeCounter++) % 40 == 0 || nodeRefresh)) {
			...
			if (!nodeRefresh && children.size() > 0) {
				for (WeakReference<TileVisNode> n : children) {
					if (n == null || n.get() == null
							|| !VisNetHandler.canNodeBeSeen(this, n.get())) {
						nodeRefresh = true;
						break;
					}
				}
	...
	/**
	 * @return the type of shard this is attuned to: none -1, air 0, fire 1,
	 *         water 2, earth 3, order 4, entropy 5 Should return -1 for most
	 *         implementations
	 */
