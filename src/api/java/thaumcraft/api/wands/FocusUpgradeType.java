...
public class FocusUpgradeType {
	...
	/**
	 * What aspects are used to calculate the cost of this upgrade. The amounts
	 * given is ignored, just the type is used for the calculation.
	 */
	public AspectList aspects;

	public FocusUpgradeType(int id, ResourceLocation icon, String name,
			String text, AspectList aspects) {
		this.id = (short) id;
		this.icon = icon;
		this.name = name;
		this.text = text;
		...
		if (id < types.length && types[id] != null) {
			LogManager.getLogger("THAUMCRAFT").fatal(
					"Focus Upgrade id " + id + " already occupied. Ignoring.");
			return;
		}
	...
	public static FocusUpgradeType alchemistsfire = new FocusUpgradeType(4,
			...
			"focus.upgrade.alchemistsfire.text", new AspectList().add(
					Aspect.ENERGY, 1).add(Aspect.SLIME, 1));
	...
	public static FocusUpgradeType alchemistsfrost = new FocusUpgradeType(5,
			...
			"focus.upgrade.alchemistsfrost.text", new AspectList().add(
					Aspect.COLD, 1).add(Aspect.TRAP, 1));
