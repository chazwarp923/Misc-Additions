...
public class ItemLumberAxe extends ItemAxe {
				...
				if(world.blockExists(x, y + i, z)) {
					if(world.getBlock(x, y, z).getMaterial() == Material.wood) {
						if(world.getBlock(x, y + i, z) == block) {
							...
							ArrayList<ItemStack> stackArrayList = block.getDrops(world, x, y, z, world.getBlockMetadata(x, y, z), 3);
							
							for(int i1=0; i1 < stackArrayList.size(); i1++) {
								world.spawnEntityInWorld(new EntityItem(world, x, y, z, stackArrayList.get(i1)));
							}
						}
					}
				}
	...
	public void addInformation(ItemStack stack, EntityPlayer ep, List par3List, boolean bool) {
		...
		if(!Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
			par3List.add("Hold " + EnumChatFormatting.RED + EnumChatFormatting.ITALIC + "Shift" + EnumChatFormatting.GRAY + " For Details");
		}
		else if(Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
			par3List.add(EnumChatFormatting.ITALIC + "Deforest The World! -Chazwarp923");
		}
	}
