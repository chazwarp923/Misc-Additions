...
public class CrucibleRecipe {
	...
	public CrucibleRecipe(String researchKey, ItemStack result, Object cat,
			AspectList tags) {
		recipeOutput = result;
		this.aspects = tags;
		...
		} else if (cat instanceof ArrayList
				&& ((ArrayList<ItemStack>) catalyst).size() > 0) {
	...
	public boolean matches(AspectList itags, ItemStack cat) {
		if (catalyst instanceof ItemStack
				&& !ThaumcraftApiHelper.itemMatches((ItemStack) catalyst, cat,
						false)) {
			return false;
		} else if (catalyst instanceof ArrayList
				&& ((ArrayList<ItemStack>) catalyst).size() > 0) {
			ItemStack[] ores = ((ArrayList<ItemStack>) catalyst)
					.toArray(new ItemStack[] {});
			if (!ThaumcraftApiHelper.containsMatch(false,
					new ItemStack[] { cat }, ores))
				return false;
		}
		if (itags == null)
			return false;
