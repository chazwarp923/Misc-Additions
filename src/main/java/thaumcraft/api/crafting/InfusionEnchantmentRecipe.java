...
public class InfusionEnchantmentRecipe {
	...
	public InfusionEnchantmentRecipe(String research, Enchantment input,
			int inst, AspectList aspects2, ItemStack[] recipe) {
	 ...
	 * @param player
	 */
	public boolean matches(ArrayList<ItemStack> input, ItemStack central,
		...
		if (!enchantment.canApply(central)
				|| !central.getItem().isItemTool(central)) {
			return false;
		}
		...
		while (iterator.hasNext()) {
			...
			if (j1 == enchantment.effectId
					&& EnchantmentHelper.getEnchantmentLevel(j1, central) >= ench
							.getMaxLevel())
				return false;
			if (enchantment.effectId != ench.effectId
					&& (!enchantment.canApplyTogether(ench) || !ench
	...
	protected boolean areItemStacksEqual(ItemStack stack0, ItemStack stack1,
		...
		if (stack0 == null && stack1 == null)
			return true;
		boolean t1 = ThaumcraftApiHelper.areItemStackTagsEqualForCrafting(
				stack0, stack1);
		if (!t1)
			return false;
		if (fuzzy) {
			...
			if (od != -1) {
				ItemStack[] ores = OreDictionary.getOres(od).toArray(
						new ItemStack[] {});
				if (ThaumcraftApiHelper.containsMatch(false,
						new ItemStack[] { stack1 }, ores))
					return true;
			}
		}
		return stack0.getItem() != stack1.getItem() ? false : (stack0
				.getItemDamage() != stack1.getItemDamage() ? false
	...
	public int calcXP(ItemStack recipeInput) {
		return recipeXP
				* (1 + EnchantmentHelper.getEnchantmentLevel(
						enchantment.effectId, recipeInput));
	}

	public float getEssentiaMod(ItemStack recipeInput) {
		float mod = EnchantmentHelper.getEnchantmentLevel(enchantment.effectId,
				recipeInput);
		Map map1 = EnchantmentHelper.getEnchantments(recipeInput);
		Iterator iterator = map1.keySet().iterator();
		while (iterator.hasNext()) {
