...
public class VisNetHandler {
	 ...
	 */
	public static int drainVis(World world, int x, int y, int z, Aspect aspect,
			int amount) {
		...
		if (nodes != null && nodes.size() > 0)
			for (WeakReference<TileVisNode> noderef : nodes) {
				...
				if (a > 0) {
					...
					generateVisEffect(world.provider.dimensionId, x, y, z,
							node.xCoord, node.yCoord, node.zCoord, color);
				}
				if (amount <= 0) {
					break;
				}
	...
	public static void generateVisEffect(int dim, int x, int y, int z, int x2,
			int y2, int z2, int color) {
		ThaumcraftApi.internalMethods.generateVisEffect(dim, x, y, z, x2, y2,
				z2, color);
	}
	...
	public static WeakReference<TileVisNode> addNode(World world, TileVisNode vn) {
		...
		if (nearby.size() > 0) {
			for (Object[] o : nearby) {
				if ((Float) o[1] < dist
						&& (vn.getAttunement() == -1
								|| ((TileVisNode) o[0]).getAttunement() == -1 || vn
								.getAttunement() == ((TileVisNode) o[0])
								.getAttunement())// ) {
	...
	private static ArrayList<WeakReference<TileVisNode>> getAllChildren(
		...
		for (WeakReference<TileVisNode> child : source.getChildren()) {
			...
			if (n != null && n.getWorldObj() != null
					&& isChunkLoaded(n.getWorldObj(), n.xCoord, n.zCoord)) {
				list.add(child);
	...
	public static boolean canNodeBeSeen(TileVisNode source, TileVisNode target) {
		MovingObjectPosition mop = ThaumcraftApiHelper.rayTraceIgnoringSource(
				source.getWorldObj(), Vec3.createVectorHelper(
						source.xCoord + .5, source.yCoord + .5,
						source.zCoord + .5), Vec3.createVectorHelper(
						target.xCoord + .5, target.yCoord + .5,
						target.zCoord + .5), false, true, false);
		return mop == null
				|| (mop.typeOfHit == MovingObjectType.BLOCK
						&& mop.blockX == target.xCoord
						&& mop.blockY == target.yCoord && mop.blockZ == target.zCoord);
	}
