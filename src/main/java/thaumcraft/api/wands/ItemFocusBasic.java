...
public class ItemFocusBasic extends Item {
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List list,
		...
		if (al != null && al.size() > 0) {
			list.add(StatCollector
					.translateToLocal(isVisCostPerTick(stack) ? "item.Focus.cost2"
							: "item.Focus.cost1"));
	...
	public void addFocusInformation(ItemStack focusstack, EntityPlayer player,
		...
		for (Short id : map.keySet()) {
			list.add(EnumChatFormatting.DARK_PURPLE
					+ FocusUpgradeType.types[id].getLocalizedName()
					+ (map.get(id) > 1 ? " "
							+ StatCollector
	...
	/**
	 * Does the focus have ornamentation like the focus of the nine hells.
	 * Ornamentation is a standard icon rendered in a cross around the focus
	 */
	...
	/**
	 * This returns how many milliseconds must pass before the focus can be
	 * activated again.
	 */
	...
	public boolean applyUpgrade(ItemStack focusstack, FocusUpgradeType type,
			int rank) {
	...
	/**
	 * Use this method to define custom logic about which upgrades can be
	 * applied. This can be used to set up upgrade "trees" that make certain
	 * upgrades available only when others are unlocked first, when certain
	 * research is completed, or similar logic.
	 * 
	 */
	public boolean canApplyUpgrade(ItemStack focusstack, EntityPlayer player,
			FocusUpgradeType type, int rank) {
		return true;
	}
	 ...
	 */
	public boolean isUpgradedWith(ItemStack focusstack,
			FocusUpgradeType focusUpgradetype) {
		return getUpgradeLevel(focusstack, focusUpgradetype) > 0;
	}

	/**
	 * What level is the passed upgrade type on the focus. If it is not present
	 * it returns 0
	 */
	public int getUpgradeLevel(ItemStack focusstack,
			FocusUpgradeType focusUpgradetype) {
	...
	public void onUsingFocusTick(ItemStack wandstack, EntityPlayer player,
			int count) {
		// TODO Auto-generated method stub
	}

	public void onPlayerStoppedUsingFocus(ItemStack wandstack, World world,
			EntityPlayer player, int count) {
		// TODO Auto-generated method stub

	}

	public boolean onFocusBlockStartBreak(ItemStack wandstack, int x, int y,
	...
	private NBTTagList getFocusUpgradeTagList(ItemStack focusstack) {
		return focusstack.stackTagCompound == null ? null
				: focusstack.stackTagCompound.getTagList("upgrade", 10);
	}
	...
	public void onUpdate(ItemStack stack, World world, Entity entity,
		...
		if (stack.stackTagCompound != null
				&& stack.stackTagCompound.hasKey("ench")) {
